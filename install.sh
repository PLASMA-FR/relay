...
if [ -z "$binary" ]; then
    if [ -f "$script_dir/go.mod" ]; then
        ...
        (cd "$script_dir" && CGO_ENABLED=0 go build -trimpath -buildvcs=false -ldflags '-s -w -X github.com/PLASMA-FR/relay/internal/model.Version=0.3.0' -o "$work/relay" ./cmd/relay)
        binary=$work/relay
